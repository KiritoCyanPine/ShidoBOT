...
class MangaInfo(APIBase):
    ...
    def _get_title(self) -> typing.Union[str, None]:
        ...
        Title = str(self._soup.find(class_="_3xnDj"))
        Title = Title[:Title.index("<small>")].replace("\n","")

        return Title

    def _get_icon(self) -> str:
        imageUrl = self._soup.find(class_="_4RcEi").find("img").attrs.get("src")
    ...
    def _get_chapter_list(self) -> typing.List[MangaChapter]:
        ...
        count = 1
        try:
            panels = self._soup.find(class_="tab-content")
            for i, ele in enumerate(reversed(panels.find_all(class_="list-group-item"))):
                if ele is not None:
                    url = ele.find("a")["href"]
                    text = ele.find("a").find(class_="text-secondary").text
    ...
    def _parse_table(self) -> dict:
        ...
        data = {}
        try:
            data["author"] = str(self._soup.find(class_="_3SlhO").text).replace("\n", "")
        except AttributeError:
            data["author"] = "N/A"
        try:
            data["genres"] = [ele.text for ele in self._soup.find(class_="_3Czbn").find_all("a")]
        except AttributeError:
            data["genres"] = ["N/A"]
        try:
            data["icon"] = self._soup.find(class_="_4RcEi").find("img").attrs.get("src")
        except AttributeError:
            data["icon"] = None
        try:
