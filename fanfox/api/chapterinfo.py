...
class ChapterInfo(APIBase):
	...
	def _start(self) -> None:
		...
		try:
			WebDriverWait(browser, 1).until(EC.presence_of_element_located((By.CLASS_NAME, "reader-main-img")))
		except TimeoutException:
			print("TimeoutException: Element not found")
			return None
		# print(element)
		response = browser.page_source
		self._soup: BeautifulSoup = BeautifulSoup(response, "lxml")
	...
	def _get_image_urls(cls, uniformRes) -> List[str]:
		...
		divSoup = uniformRes.find(class_="reader-main")
		# print("div soup :::", divSoup)
		image_soup = divSoup.find_all("img")

		images = [url for url in map(lambda ele: ele["data-src"], image_soup) if valid(url)]
		# print(images)
		return images
	...
	def _get_chapter_title(cls, soup: BeautifulSoup) -> str:
		...
		title = soup.find(class_="reader-header-title-2").text
