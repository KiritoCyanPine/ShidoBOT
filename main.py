...
async def on_message(own):
    ...
    try:
        ...
        if own.content.startswith('!shido> '):  # take the content : Manga Name
            ...
            msg = await client.wait_for('message', timeout=60, check=lambda message: message.author == msgAuth)
            ...
            msg1 = await client.wait_for('message', timeout=60, check=lambda message: message.author == msgAuth)  # wait for cmd
            if not start_running:
                start_running = True
                ...
                if len(chapter_list) == 0:  # if chapter is empty in fanfox .. it is protected
                    ...
                    await own.channel.send(embed=embed)
                    start_running = False
                    print(f"""Request from {msgAuth} -\n 
                                            request for manga by name : '{own.content}'\n
                                            Manga Selected : '{MangaName}'\n
                                            Server Chosen : '{serverName}'\n
                                            successful response : '{SuccessfulResponse}'\n
                                            Chapters Requested : {chapter_sChose}""")
            ...
            msg2 = await client.wait_for('message', timeout=60, check=lambda message: message.author == msgAuth)
                ...
                try:
                    if "-" not in chapter_sChose:  # if it does not contain "-" then it's request for single chapter
                        ...
                        if len(getRange) == 2 and getRange[0].isdecimal() and getRange[1].isdecimal():
                            ...
                            getRange = list(map(int, getRange))
                            lower = min(getRange)
                            higher = max(getRange)
        ...
        msg = await own.channel.send("❌ It seems You took too long to respond... So, I'm restarting the session ")
        await msg.add_reaction("❌")
        print("Response TimeOut ... Session Ended...")
        return
